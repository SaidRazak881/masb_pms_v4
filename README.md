...
| `docs/PROMPT-7A-FIX-FIELD-MAPPING.md` | **🟡 MENUNGGU KELULUSAN PENGGUNA (HARD GATE).** Pasang `fix-field-mapping.sql` (23 lajur baharu) + ganti RPC `sync_import_transaction`. Kriteria J1–J10, snapshot SEBELUM diarahkan eksplisit (pengajaran #10), pengesahan guna `pg_proc.prosrc` bukan `action_statement` (pengajaran #8). **Sengaja TIDAK** meminta ujian berfungsi di live — sudah diuji 55/55 dalam PGlite |
