...
| `lib/supabase/updated-at-triggers.sql` | **Fasa 6G — ⏸️ DITANGGUH oleh pengguna (2026-09-04), BELUM dipasang di live. WAJIB untuk pemasangan bersih.** Membetulkan kecacatan repo: repo mencipta kolum `updated_at` pada 10 jadual rasmi tetapi **tidak pernah** mencipta trigger. Cipta `public.set_updated_at()`, alih 6 trigger pra-repo, tambah 6 trigger baharu → 12 jadual. Idempoten, tidak DROP apa-apa |
| `docs/PROMPT-6G-UPDATED-AT-AND-REVOKE.md` | **⏸️ DITANGGUH oleh pengguna (2026-09-04) — jangan jalankan.** Kandungan masih sah jika diaktifkan semula: pasang `updated-at-triggers.sql` (G1–G3) + REVOKE privilej tulis 3 jadual warisan (H1–H3, SELECT dikekalkan) + laporan I1–I2 tentang `validate_programme_lock`. Mengandungi **pembetulan Arena**: `private.has_role()` TIDAK membolehkan escalation (WITH CHECK menolak INSERT) |
...
| `node scripts/test-updated-at-triggers.mjs` | Sahkan `updated-at-triggers.sql`: tiru keadaan live (6 trigger pra-repo), pasang, bukti **berkelakuan** bahawa `updated_at` berubah pada UPDATE (termasuk jadual yang dahulunya tiada trigger), idempoten, tidak DROP apa-apa |
