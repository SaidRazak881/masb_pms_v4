# PROMPT 8A-3 / L3 — Langkah 3 — `account-manager-resolution.sql` (Fasa 8A-2)
...
**Prasyarat:** **Langkah 1 dan 2 mesti sudah dipasang.** `am_confirm_external()` menulis ke jadual dari Langkah 2.

**Objek yang dipasang langkah ini:** 7 fungsi: permukaan untuk manusia mengesahkan alias (`am_confirm_alias`, `am_revoke_alias`), mengklasifikasi orang luar (`am_confirm_external`, `am_revoke_external`, `is_external_account_manager`), dan `am_backfill_account_manager()` + `am_backfill_preview()`.

> 🔴 **JANGAN panggil `am_backfill_account_manager()` dalam langkah ini.**
...
|---|---|
| **Lapis 1 — Git blob SHA** | `afcdc600efda41bc4e1928c60fe71dd6be2880ba` |
| Lapis 2 — bait (UTF-8) | **21276** |
| Lapis 2 — baris | **539** |
| Lapis 2 — aksara (titik kod Unicode) | **21237** |
| Lapis 2 — `CREATE TABLE` / `FUNCTION` / `POLICY` / `INDEX` | **0 / 7 / 0 / 0** |
| Lapis 2 — baris pertama | `-- =====================================================================` |
| Lapis 2 — baris terakhir (bukan kosong) | `-- Ujian berkelakuan: scripts/test-account-manager-resolution.mjs` |
| Pilihan — SHA-256 | `fb32d1d00f89322dd091f70df82984196c007b1b2040b79823c2ea5073752120` |
...
separa. Rujuk fail asal untuk perbandingan: https://github.com/SaidRazak881/masb_pms_v4/blob/arena/01a06274-masb-pms-v4/lib/supabase/account-manager-resolution.sql
...
seperti ada. Fail asal: `lib/supabase/account-manager-resolution.sql`
...
2. Hantar **keseluruhan** teks SQL di atas kepada `Supabase.apply_migration`
   ...
   Nama cadangan: `8a3-l3-account-manager-resolution`.
...
-- L3c: fungsi baharu wujud
SELECT 'L3c' AS check_name, p.proname AS fungsi,
       pg_get_function_identity_arguments(p.oid) AS argumen
  FROM pg_proc p
 WHERE p.pronamespace = 'public'::regnamespace
   AND p.proname IN ('can_resolve_account_managers', 'am_list_staff', 'am_unresolved_values', 'am_confirm_alias', 'am_revoke_alias', 'am_backfill_preview', 'am_backfill_account_manager')
 ORDER BY p.proname;
-- Jangkaan: 7 baris

---

### L3v — versi platform (read-only, 🔴 LAPORKAN)
```sql
...
`account-manager-resolution.sql` **sudah dipasang**, dan sertakan cap jari yang anda sahkan
